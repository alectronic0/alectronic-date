...
(function () {
  ...
  const countryName = (cc) => {
    try { return (regionNames && regionNames.of(cc.toUpperCase())) || cc.toUpperCase(); } catch (e) { return cc.toUpperCase(); }
  };
...
<strong>${
    ...
    pills: (b) =>
      ...
      b.pills.map((f) => {
        ...
        return `<span class="fact"${toastAttr} title="${esc(f.label)}"><span class="fact-icon" aria-hidden="true">${flagify(
          f.icon
      ...
      setTimeout(() => {
        ...
        if (mapEl && !mapEl._leaflet_id) {
          ...
          const line = L.polyline([
            p1,
            p2
          ], {color: '#e6a979', dashArray: '5, 5', weight: 2}).addTo(map);
  ...
  function revealSecret(mode = 'normal') {
    ...
    const emojis = isParty
      ...
      : ((C && C.easterEgg && C.easterEgg.emojis) || [
        '❤️',
        '🍄',
        '⭐',
        '🎮'
