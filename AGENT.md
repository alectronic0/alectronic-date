...
To avoid duplicating the site layout across multiple pages (`index.html`, `404.html`, etc.), the `app.js` script dynamically injects the shared `<header>`, `<nav>`, and `<footer>` elements on load. Pages only need to include the core layout tags or wait for `app.js` to build them.
